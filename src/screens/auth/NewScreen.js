...
import { SafeAreaView, Text, StyleSheet, FlatList, View, TouchableOpacity , TextComponent, TextInput} from 'react-native';
...
const data = {
 ...
 export default NewScreen = () => {
    ...
    useEffect(() => {
        ...
        async function filterCateogries () {
        if(searchItem.trim() !== ""){
            const result = await data?.categorys.map((category) => {
                const obj = category;
               const res = category["menu-items"].filter((item) => {
                    return searchItem == item.name;
                });
                obj["menu-items"] = res;
                return obj;
            })

            console.log("result__new______",result)

            await setCategoryData(result);
        }else{
            await setCategoryData(data?.categorys)
        }
    }
    filterCateogries();
    },[searchItem])
    ...
    const _renderCategories = ({item}) => {
        const menuItems = item["menu-items"];
        console.log("mendu_items_______",menuItems)
        return(
            <TouchableOpacity onPress={() => setSelectedCategory(item.id)} style={styles.categoryContainer}>
             <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                <View style={{flexDirection: 'row'}}>
                  <Text>{item.name}</Text>
                  <Text>{menuItems ?  " (" + menuItems.length + ")" : "0"}</Text>
                </View>
                <Icon name='chevron-down-outline' size={getFontSize(18)} />
             </View>   
        ...
        <SafeAreaView style={{flex: 1}}>
            ...
            <View style={styles.searchBar}>
                <TextInput
                 ...
                 style={{flex: 1}} />
                <Icon name='search-outline' size={getFontSize(18)} />
            </View>
